...
```
testmu-jmeter-studio-1.2.0-share.zip          ←  the one you want
  └── testmu-jmeter-studio/
        manifest.json
        background.js
        popup.html …

testmu-jmeter-studio-1.2.0.zip                ←  only for a Chrome Web Store submission
  ├── manifest.json
  ├── background.js
  ├── popup.html …
```
...
`testmu-jmeter-studio-1.2.0-share.zip`, then **Download raw file** at the top right.
...
| The recording seems to have vanished | Chrome evicts idle service workers. The session is checkpointed to storage and restored, so reopen the popup and check the counter before assuming it is gone |
